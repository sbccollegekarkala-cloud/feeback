...
const Storage = {
    ...
    /**
     * Find user by email (OPTIMIZED - uses Firestore query)
     */
    ...
    /**
     * Find user by username (OPTIMIZED - uses Firestore query)
     */
    ...
    async getFeedbacksByStudentId(studentId) {
        try {
            ...
            // OPTIMIZED: Query only this student's feedbacks
            const feedbacksRef = collection(db, this.COLLECTIONS.FEEDBACKS);
    ...
    async getFeedbacksBySurveyId(surveyId) {
        try {
            ...
            // OPTIMIZED: Query only this survey's feedbacks
            const feedbacksRef = collection(db, this.COLLECTIONS.FEEDBACKS);
            const q = query(feedbacksRef, where('surveyId', '==', surveyId));
            const snapshot = await getDocs(q);
